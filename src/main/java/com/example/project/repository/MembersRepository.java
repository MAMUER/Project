...
public interface MembersRepository extends JpaRepository<Members, Integer> {
    ...
    Set<Members> findByEmail(String email);

    Set<Members> findByPhoneNumber(String phoneNumber);

    Set<Members> findByClubClubName(String clubName);

    Set<Members> findByMembershipRoleIdRole(Integer roleId);

    Set<Members> findByBirthDateBetween(LocalDate startDate, LocalDate endDate);

    Set<Members> findByEndTrialDateIsNullOrEndTrialDateAfter(LocalDate currentDate);

    Set<Members> findByGender(Integer gender);

    @Query(value = "SELECT COUNT(*) > 0 FROM members_have_training_schedule WHERE member_id = :memberId AND training_id = :trainingId", nativeQuery = true)
