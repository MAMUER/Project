-- Active: 1757597241829@@127.0.0.1@5432@fitness_club_db@fitness_club_db
-- Создание пользователя, если его нет
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT FROM pg_catalog.pg_roles
        WHERE rolname = 'springroot'
    ) THEN
...
-- Предоставление всех привилегий на все существующие таблицы в схеме
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA fitness_club_db TO springroot;

-- Установка прав на все будущие таблицы, которые будут создаваться в схеме
...
-- Заполняем таблицу требований оборудования для упражнений
-- Заполняем таблицу требований оборудования для упражнений
INSERT INTO exercise_equipment_requirements (id_exercise, id_equipment_type, quantity_required, is_required) VALUES
-- Для упражнений со штангой
(1, 1, 1, true),  -- Жим штанги лежа -> Штанга
(5, 1, 1, true),  -- Становая тяга -> Штанга
(6, 1, 1, true),  -- Тяга штанги в наклоне -> Штанга
(9, 1, 1, true),  -- Приседания со штангой -> Штанга
(13, 1, 1, true), -- Жим штанги стоя -> Штанга
(14, 1, 1, true), -- Тяга штанги к подбородку -> Штанга
(16, 1, 1, true), -- Подъем штанги на бицепс -> Штанга
(17, 1, 1, true), -- Французский жим -> Штанга
-- Для упражнений с гантелями
(2, 1, 2, true),  -- Жим гантелей лежа -> Гантели (2 шт)
(11, 1, 2, true), -- Выпады с гантелями -> Гантели (2 шт)
(15, 1, 2, true), -- Махи гантелями -> Гантели (2 шт)
(18, 1, 2, true), -- Молотковые сгибания -> Гантели (2 шт)
-- Для упражнений на брусьях
(3, 2, 1, true),  -- Отжимания на брусьях -> Брусья
-- Для кардио упражнений
(19, 4, 1, true), -- Бег на дорожке -> Кардио тренажеры
(20, 4, 1, true), -- Велотренажер -> Кардио тренажеры
(21, 4, 1, false), -- Скакалка -> Скакалки (не обязательно)
(22, 4, 1, true); -- Эллиптический тренажер -> Кардио тренажеры

SELECT id_exercise FROM exercises ORDER BY id_exercise;;
SELECT * from members;

SELECT * from members_have_training_schedule;

SELECT * from clubs_have_news;